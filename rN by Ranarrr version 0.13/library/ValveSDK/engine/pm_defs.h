...
struct playermove_s {
	...
	qboolean		server;        // For debugging, are we running physics code on server side?					0x004

	qboolean		multiplayer;   // 1 == multiplayer server														0x008
	...
	int( *PM_TestPlayerPosition ) ( Vector pos, pmtrace_t *ptrace );
	void( *Con_NPrintf )( int idx, char *fmt, ... );
	void( *Con_DPrintf )( char *fmt, ... );
	void( *Con_Printf )( char *fmt, ... );
	double( *Sys_FloatTime )( void );
	void( *PM_StuckTouch )( int hitent, pmtrace_t *ptraceresult );
	int( *PM_PointContents ) ( Vector p, int *truecontents /*filled in if this is non-null*/ );
	int( *PM_TruePointContents ) ( Vector p );
	int( *PM_HullPointContents ) ( struct hull_s *hull, int num, Vector p );
	pmtrace_t( *PM_PlayerTrace ) ( Vector start, Vector end, int traceFlags, int ignore_pe );
	struct pmtrace_s *( *PM_TraceLine )( float *start, float *end, int flags, int usehull, int ignore_pe );
	long( *RandomLong )( long lLow, long lHigh );
	float( *RandomFloat )( float flLow, float flHigh );
	int( *PM_GetModelType )( struct model_s *mod );
	void( *PM_GetModelBounds )( struct model_s *mod, Vector mins, Vector maxs );
	void			*( *PM_HullForBsp )( physent_t *pe, Vector offset );
	float( *PM_TraceModel )( physent_t *pEnt, Vector start, Vector end, trace_t *trace );
